...
public class BuildInvertedIndexCompressed extends Configured implements Tool {
  ...
  private static final class MyMapper extends Mapper<LongWritable, Text, PairOfStringInt, IntWritable> {
    ...
    public void map(LongWritable docno, Text doc, Context context)
      ...
      for (PairOfObjectInt<String> e : COUNTS) {
        ...
        // the value 0 make sure that after partition, these special pairs are always at top
        WORD.set(e.getLeftElement(), 0);
  ...
  private static final class MyReducer extends
    ...
    private static final ByteArrayOutputStream BYTEARRAYOUTPUT = new ByteArrayOutputStream();
    private static final DataOutputStream DATAOUTPUT = new DataOutputStream(BYTEARRAYOUTPUT);
    private static int docIDPrev = 0;
    ...
    public void reduce(PairOfStringInt key, Iterable<IntWritable> values, Context context)
      ...
      if (key.getRightElement() == 0) {
        // if ByteArrayOutPutStream not empty, emit the previous data there
        if (BYTEARRAYOUTPUT.size() != 0) {
          context.write(WORD, new BytesWritable(BYTEARRAYOUTPUT.toByteArray()));
          BYTEARRAYOUTPUT.reset();
          DATAOUTPUT.flush();
          docIDPrev = 0;
        }
        // getting df, always first than the actual posting list
        int df = 0;
        while (iter.hasNext()) {
          df += iter.next().get();
        }
        // write the df as VInt, set current posting list's key as key read
        WritableUtils.writeVInt(DATAOUTPUT, df);
        WORD.set(key.getLeftElement());
      } else {
        // reading the whole list and writing docID - docIDPrev and word count consecutively as VInts to the outputstream 
        while (iter.hasNext()) {
          WritableUtils.writeVInt(DATAOUTPUT, key.getRightElement() - docIDPrev);
          WritableUtils.writeVInt(DATAOUTPUT, iter.next().get());
          docIDPrev = key.getRightElement();
        }
      }
    }
